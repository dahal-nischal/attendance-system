...
function Dashboard() {
  ...
  const handleCardScan = async (userId) => {
    try {
      const userRecord = attendance.find(record => record.user_id === userId);

      if (userRecord && userRecord.out_time) {
        // Show toast notification if out time is already recorded
        toast.current.show({
          severity: 'warn',
          summary: 'Out Time Recorded',
          detail: 'Out time is already recorded for this user.',
          life: 3000
        });
        return;
      }

      // Proceed with the card scan logic
      // ...
